...
namespace AsistenciaSalud
{
    ...
    public partial class Principal : MetroForm
    {
        ...
        public Principal(Clases.Usuario User)
        {
            InitializeComponent();
            this.Usuario = User;
        }
        ...
        private void mlfaltas_Click(object sender, EventArgs e)
        {
            Forms.Faltas formFaltas = new Forms.Faltas(/*this.Usuario*/);
            formFaltas.Show();
            //this.Hide();
        }
        ...
        {
            tslbpersonal.Text = "Bienvenido " + this.Usuario.nombre +" "+ this.Usuario.apellido_paterno + " " + this.Usuario.apellido_materno;
            tslclue.Text = "Esta conectado a la Unidad " + this.Usuario.Unidades[this.Usuario.unidadSeleccionada].unidad;

            //mlpermisos.Enabled = true;

            if (this.Usuario.superUser == "1")
            {
            ...
            { DatosMaestro consultar = new DatosMaestro();
                DataTable dt;  
                dt= consultar.ConsultarPermisosUsurio(this.Usuario.id);
